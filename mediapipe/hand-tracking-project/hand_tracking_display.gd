...
@onready var btn_reload: Button = $SelectCamera/VBoxContainer/HBoxContainer/ButtonReload
...
func _setup_camera_selection():
	...
	# Connexion du bouton Reload
	btn_reload.pressed.connect(self._on_reload_pressed)
...
func _on_reload_pressed():
	print("Recherche de nouvelles caméras...")
	# On force le monitoring à false puis true pour déclencher un scan système
	CameraServer.monitoring_feeds = false
	await get_tree().create_timer(0.1).timeout
	CameraServer.monitoring_feeds = true
	
	# On vide les sélecteurs avant de recharger
	selected_feed.clear()
